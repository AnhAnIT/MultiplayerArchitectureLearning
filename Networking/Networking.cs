...
namespace MultiplayCore
{
	...
	public class Networking : MonoBehaviour
	{
		...
		public void StartGame(SessionRequest request)
		{
			...
			// Build fix: Try to prepend Assets/ and append .unity if partial path fails
			if (sceneIndex < 0)
			{
				string fullPath = $"Assets/{request.ScenePath}.unity";
				sceneIndex = SceneUtility.GetBuildIndexByScenePath(fullPath);
				if (sceneIndex >= 0) Log($"Resolved scene index {sceneIndex} using full path: {fullPath}");
			}
			...
			{
				Debug.LogError($"FAILED to resolve scene index for path: {request.ScenePath}. Build index might be missing!");
			}
